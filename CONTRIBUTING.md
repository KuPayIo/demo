...
There are opportunities to contribute to Demo at any level. It doesn't matter if
...
The Demo project adheres to the [Rust Code of Conduct][coc]. This describes
...
[ynbot@yinengyun.com](mailto:ynbot@yinengyun.com).
...
1. By opening the issue for discussion: For instance, if you believe that you
   have discovered a bug in Demo, creating a new issue in [the issue][issue] is the way to report it.
...
3. By helping to resolve the issue: Typically this is done either in the form of
   ...
   Demo in a concrete and reviewable manner.

[issue]: https://github.com/KuPayIo/demo/issues
...
[open a discussion]: https://github.com/KuPayIo/demo/discussions

### Submitting a Bug Report

When opening a new issue in the Demo issue tracker, you will be presented
...
cases should be limited, as much as possible, to using only Demo APIs.
...
functional guidelines of the Demo project.

## Pull Requests

Pull Requests are the way concrete changes are made to the code, documentation,
and dependencies in the Demo repository.
...
Due to the extensive use of features in Demo, you will often need to add extra
...
Clippy must be run using the MSRV, so Demo can avoid having to `#[allow]` new
...
- Demo/README.md
- Demo/Cargo.toml
- Demo-util/Cargo.toml
- Demo-test/Cargo.toml
- Demo-stream/Cargo.toml
-->
...
required for various parts of Demo are missing. To build the documentation
...
To build documentation including Demo's unstable features, it is necessary to
pass `--cfg Demo_unstable` to both RustDoc *and* rustc. To build the
documentation for unstable features, use this command:

```
RUSTDOCFLAGS="--cfg docsrs --cfg Demo_unstable" RUSTFLAGS="--cfg docsrs --cfg Demo_unstable" cargo +nightly doc --all-features
```

The `cargo fmt` command does not work on the Demo codebase. You can use the
...
```
cd Demo # Demo crate in workspace
...
example), it is either adding new functionality to Demo or it is fixing
existing, broken functionality. In both of these cases, the pull request should
include one or more tests to ensure that Demo does not regress in the future.
There are two ways to write tests: integration tests and documentation tests
(Demo avoids unit tests as much as possible).

#### Integration tests

Integration tests go in the same crate as the code they are testing. Each sub
crate should have a `dev-dependency` on `Demo` itself. This makes all Demo
...
`Demo` crate is available. This is especially useful for getting access to the
runtime to run the example.

The documentation tests will be visible from both the crate specific
documentation **and** the `Demo` facade documentation via the re-export. The
example should be written from the point of view of a user that is using the
`Demo` crate. As such, the example should use the API via the facade and not by
directly referencing the crate.

The type level example for `Demo_timer::Timeout` provides a good example of a
documentation test:

```
/// // import the `timeout` function, usually this is done
/// // with `use Demo::prelude::*`
/// use Demo::prelude::FutureExt;
/// use futures::Stream;
...
/// });
///
/// # Demo::runtime::current_thread::block_on_all(
...
of `Demo` will create an instance of `Timeout` is by using
...
1. The first line should:
  ...
  * be prefixed with the name of the sub crate being changed (without the `Demo-`
    prefix) and start with an imperative verb. If modifying `Demo` proper,
    omit the crate prefix.

  Examples:

  * timer: introduce `Timeout` and deprecate `Deadline`
  * export `Encoder`, `Decoder`, `Framed*` from Demo_codec
...
**Any Demo community member is welcome to review any pull request**.

All Demo contributors who choose to review and provide feedback on Pull
...
1. Does this change make sense for Demo?
2. Does this change make Demo better, even if only incrementally?
...
avoid stalling the Pull Request. Most nits can typically be fixed by the Demo
...
a particular change that makes Demo better, but the individual might just not
want to have anything to do with Demo ever again. The goal is not just having
good code.
...
The Demo GitHub repository has a lot of issues and PRs to keep track of. This
...
The area label describes the crates relevant to this issue or PR.
 ...
 - **A-Demo** This issue concerns the main Demo crate.
 - **A-Demo-util** This issue concerns the `Demo-util` crate.
 - **A-Demo-tls** This issue concerns the `Demo-tls` crate. Only used for
   older issues, as the crate has been moved to another repository.
 - **A-Demo-test** The issue concerns the `Demo-test` crate.
 - **A-Demo-macros** This issue concerns the `Demo-macros` crate. Should only
...
The module label provides a more fine grained categorization than **Area**.
 ...
 - **M-codec** The `Demo_util::codec` module.
 - **M-compat** The `Demo_util::compat` module.
 - **M-coop** Things relevant to coop.
 - **M-fs** The `Demo::fs` module.
 - **M-io** The `Demo::io` module.
 - **M-macros** Issues about any kind of macro.
 - **M-net** The `Demo::net` module.
 - **M-process** The `Demo::process` module.
 - **M-runtime** The `Demo::runtime` module.
 - **M-signal** The `Demo::signal` module.
 - **M-sync** The `Demo::sync` module.
 - **M-task** The `Demo::task` module.
 - **M-time** The `Demo::time` module.
 - **M-tracing** Tracing support in Demo.
...
Some extra information.
 ...
 - **T-v0.1.x** This is about old Demo.

Any label not listed here is not in active use.

[project]: https://github.com/orgs/Demo-rs/projects/1

## LTS guarantees

Demo ≥1.0.0 comes with LTS guarantees:
 ...
 * All Demo ≥1.0.0 releases will support at least a 6-month old Rust
   compiler release.
...
Since the Demo project consists of a number of crates, many of which depend on
...
1. **Ensure that the release crate has no path dependencies.** When the HEAD
   version of a Demo crate requires unreleased changes in another Demo crate,
...
4. **Update the changelog for the crate.** Each crate in the Demo repository
